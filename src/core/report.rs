...
pub fn render(i: &ReportInputs) -> String {
    ...
    push(
        &mut out,
        "\nGenerated by modelsteward. The numbers below come from this \
         machine, not from a spec sheet — server-timed generation and \
         llama-bench baselines. Two honest caveats, because the details \
         matter when you compare them with yours:\n\n\
         - **settled ctx** is llama.cpp's own `--fit` projection, taken \
           against the VRAM free at load time (and less its 1024 MiB \
           safety margin) — not the largest context the card can serve. \
           The same model settles a few percent differently on a busier \
           day, which is why free VRAM is recorded beside it.\n\
         - **tg t/s** is generation from an EMPTY KV cache. **tg@depth** \
           is generation with a real conversation already in the cache, \
           which is what you get mid-session — typically 20-30% lower. \
           Compare like with like.\n\n\
         Shared by the machine's owner after review; paths are sanitized."
            .to_string(),
    );
    ...
    push(
        &mut out,
        "| model | settled ctx | free VRAM | pp t/s | tg t/s (empty) | tg t/s @depth | tool calls | bench build |"
            .to_string(),
    );
    ...
    for (id, m) in i.measurements {
        ...
        push(
            ...
            format!(
                ...
                f(m.pp_tps),
                f(m.tg_tps),
                match (m.tg_deep_tps, m.tg_depth) {
                    (Some(t), Some(d)) => format!("{t:.0} @{d}"),
                    _ => "—".into(),
                },
                match m.tool_call {
                    Some(true) => "yes",
...
mod tests {
    ...
    #[test]
    fn the_shareable_report_does_not_overclaim_or_conflate_the_two_speeds() {
        // modellab handoff 2026-09-02, issue 3: the preamble said every
        // number was "measured … not estimated" while settled ctx is a
        // --fit PROJECTION, and the table put ctx and an empty-cache
        // tg/s in one row — a sentence that was false by 24%. This is
        // the bundle users post publicly, so the claim travels.
        let mut m = crate::core::router::Measurement {
            n_ctx: Some(110_080),
            tool_call: Some(true),
            pp_tps: Some(1063.0),
            tg_tps: Some(38.25),
            tg_deep_tps: Some(28.99),
            tg_depth: Some(32_768),
            free_vram_mib: Some(22_900),
            bench_build: Some(10760),
            ..Default::default()
        };
        m.args_fp = Some("aaaa".into());
        let measurements: crate::core::router::Measurements =
            [("qwen".to_string(), m)].into_iter().collect();
        ...
        // The preamble must NOT claim the projection is a measurement.
        assert!(
            !text.contains("not estimated"),
            "the overclaiming sentence is gone:\n{text}"
        );
        assert!(text.contains("projection"), "and says what it is:\n{text}");
        // Both generation numbers appear, distinguishably.
        assert!(text.contains("tg t/s (empty)"), "{text}");
        assert!(text.contains("28 @32768") || text.contains("29 @32768"), "{text}");
        // Every table row has the same column count as its header.
        let rows: Vec<&str> = text
            .lines()
            .filter(|l| l.starts_with('|') && !l.contains("---"))
            .collect();
        assert!(rows.len() >= 2, "header + at least one model:\n{text}");
        let cols = |l: &str| l.matches('|').count();
        for r in &rows {
            assert_eq!(cols(r), cols(rows[0]), "ragged table row: {r}");
        }
    }
